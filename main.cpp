...
hittable_list random_scene() {
	...
	for (int a = -11; a < 11; a++)
		for (int b = -11; b < 11; b++) {
			auto choose_mat = random_float();//随机材质
			point3 center(a + 0.9 * random_float(), 0.2, b + 0.9 * random_float());//随机中心
...
int main(int argc, char* argv[]) {
	...
	//world.add(make_shared<sphere>(point3(-1.0, 0.0, -1.0), -0.45, material_left));//空心球
	...
	for (int i = image_height -1; i >= 0 ; i--)
	{
		...
		for (int j = 0; j < image_width; j++)
		{
			...
			auto u = (j + random_float()) / (image_width - 1);
			auto v = (i + random_float()) / (image_height - 1);
